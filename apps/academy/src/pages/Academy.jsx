...
/**
 * Synckaiden / Atlas Academy 2026 (MVP)
...
const PRO_PROMPTS = {
  appBuilder: {
    title: "Free ‘App-Builder’ Prompt (ship a production MVP fast)",
    body: `You are a senior full-stack engineer.
Goal: generate a production-ready web app called “Synckaiden Academy” with these constraints:

TECH (FREE):
- Vite + React + React Router
- TailwindCSS
- LocalStorage persistence (no DB required for MVP)
- Deploy target: Cloudflare Pages (SPA) – build output: dist

FEATURES:
1) Routes
- / (Home)
- /academy (the interactive trainer)
- /embed (no header/footer; for embedding on my site)
- /admin (local-only key generator; MVP license)

2) Academy App (/academy)
- Learn tab: modular lessons with checklists + notes
- Ship tab: workflow blueprint builder + export to Markdown
- 25 Gigs tab: list + filters
- Social tab: ethical autopilot planning (no account takeovers)
- Intel Packs tab: import/export JSON trend cards
- Pro Vault tab: locked content until access key validated

3) Pro Unlock (client-only MVP)
- Stripe Payment Link URL comes from env: VITE_STRIPE_PAYMENT_LINK
- Access key verification is done locally (WebCrypto HMAC). Key format: SYNK1.<payload>.<sig>
- Store unlock in LocalStorage with expiration.

4) UI requirements
- Minimal, clean aesthetic, large rounded corners, good spacing.
- No placeholder text like “lorem ipsum”.
- Make sure it runs with: npm install && npm run dev.

DELIVERABLES:
- Full source code with file tree.
- Clear README with local dev + deploy steps.
- Include a sample .env.example.

Important policies:
- Do NOT include any instructions for hacking or taking over accounts.
- No automated withdrawals or anything that touches money without human approval.

Now generate the full project.`,
  },
  workflowFactory: {
    title: "Workflow Factory Prompt (turn any business into an automation library)",
    body: `You are an automation architect.
Goal: design a ‘plug-and-play’ workflow factory that turns any business process into safe automations.

OUTPUT FORMAT:
Return a single JSON object with:
- intake_questions (max 15)
- system_map (entities, data sources, tools)
- workflow_templates (at least 10) each with trigger, steps, approvals, audit_log, rollback, metrics
- risk_register (top 10 risks + mitigations)
- deployment_plan (self-hosted + cloud options)
- pricing_model (setup + monthly + % of savings)

CONSTRAINTS:
- Human approval before: payments, account changes, identity actions, PHI/PII exports.
- Every action logged with timestamp, input, output, and evidence.
- Cost caps, rate limits, and a kill switch.
- Designed to be embedded into a website (synckaiden.com) as an interactive form.

EXAMPLES OF WORKFLOWS TO INCLUDE:
- Subscription bloat cleanup
- Vendor renegotiation scripts + follow-ups
- Invoice intake → approvals → payment reminders
- Review/reputation triage
- Staff scheduling smoothing
- Compliance binder builder (non-legal advice)

Now generate the JSON and include 3 example filled-in workflows: med-spa, restaurant, and real estate team.`,
  },
};
...
const SIDE_GIGS_25 = [
  {
    ...
    risk: "Unauthorized practice of law—position as ‘risk analysis’ + recommend counsel.",
  },
  {
    id: "sg-2",
    name: "Personal AI CFO for High-Net-Worth Families",
    buyer: "Wealthy households",
    what: "Bill/expense triage, subscription hunting, cashflow forecasts, tax doc prep checklists.",
    whyNow: "Multimodal documents + bank export parsing.",
  ...
  },
  {
    ...
    whyNow: "Agents + time-series + automation.",
    pricing: "$500–$5,000/mo",
    risk: "Bad decisions without oversight—must include approvals.",
  },
  {
    id: "sg-4",
    ...
    whyNow: "Tool-using agents + vendor templates.",
    pricing: "% of savings (10–25%)",
    risk: "Vendor ToS—use allowed sources.",
  },
  {
    id: "sg-5",
    name: "Local ‘Edge AI Box’ for Privacy",
    buyer: "Law firms, clinics",
    what: "On-prem: private chat + doc Q&A + call summaries.",
    whyNow: "Smaller models + better hardware.",
  ...
  },
  {
    ...
    whyNow: "Multimodal + timeline building.",
    pricing: "$99–$999",
    risk: "No fraud; factual only.",
  },
  {
    id: "sg-11",
    ...
    pricing: "$49–$499",
    risk: "Coaching only; don’t misrepresent identity.",
  },
  {
    id: "sg-12",
    ...
    pricing: "$500–$5,000",
    risk: "No hacking; user-authorized only.",
  },
  {
    id: "sg-13",
    ...
    risk: "Copyright + brand consistency.",
  },
  {
    id: "sg-14",
    name: "AI ‘Executive Assistant for Realtors’",
    buyer: "Realtors",
    what: "Listing intake, captions, comps packs, showing coordination.",
  ...
  },
  {
    ...
    risk: "Human verification required.",
  },
  {
    id: "sg-16",
  ...
  },
  {
    ...
    risk: "Don’t overpromise.",
  },
  {
    id: "sg-18",
    name: "AI ‘Time-Value Marketplace’ (legal-safe)",
    buyer: "Communities",
    what: "A time-credit system for services; more like loyalty/volunteering than money.",
    pricing: "$5–$20/user/mo",
    risk: "Avoid treating as currency; watch regulations.",
  },
  {
    id: "sg-19",
    name: "AI ‘Stress-Test My Business’ Simulator",
    buyer: "Founders",
    what: "Simulates shocks; outputs contingency plan.",
    pricing: "$199–$2,500",
    risk: "Model assumptions; label clearly.",
  },
  {
    id: "sg-20",
    ...
    risk: "Jurisdiction-aware; keep updated.",
  },
  {
    id: "sg-21",
...
const TOOL_STACK = [
  ...
  },
  {
    ...
    items: [
      { name: "OpenAI (Responses/Codex)", why: "Best-in-class coding + tool use" },
      { name: "Anthropic / Google / open models", why: "Redundancy + strengths" },
...
function Pill({ children }) {
  return <span className="inline-flex items-center rounded-full bg-muted px-3 py-1 text-xs font-medium">{children}</span>;
}
...
export default function Academy() {
  ...
  function addIntelPack(pack) {
    setState((s) => ({
      ...s,
      intelPacks: [{ id: uid(), createdAt: new Date().toISOString(), ...pack }, ...(s.intelPacks || [])].slice(0, 50),
    }));
  }
  ...
  function generateDailyPlan(minutes) {
    ...
    const pending = CORE_MODULES.flatMap((m) =>
      m.lessons.filter((l) => !(state.mastery?.[m.id]?.[l.id])).map((l) => ({ module: m, lesson: l }))
    );
      ...
      <div className="mx-auto max-w-6xl px-4 py-6">
        ...
        >
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <label className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-border bg-background px-3 py-2 text-sm shadow-sm">
              ...
              <Button variant={proUnlocked ? "secondary" : "default"} onClick={onOpenPro} className="gap-2">
                <KeyRound className="h-4 w-4" /> {proUnlocked ? "Pro unlocked" : "Unlock Pro"}
              </Button>
            </div>
          </div>
            ...
            <CardContent className="p-5">
              <div className="grid gap-4 md:grid-cols-3">
                ...
                <div>
                  ...
                  <div className="mt-2 text-xs text-muted-foreground">One meaningful win per day. Consistency beats intensity.</div>
                </div>
                <div>
                  ...
                  <div className="mt-3 flex gap-2">
                    ...
                    <Button variant="secondary" onClick={() => setState((s) => ({ ...s, profile: { ...s.profile, timeBudgetMin: dailyMinutes } }))}>
                      Save default
                    </Button>
                  </div>
                </div>
        ...
        <Tabs value={tab} onValueChange={setTab} className="w-full">
          <TabsList className="grid w-full grid-cols-2 md:grid-cols-6">
            ...
            <TabsTrigger value="vault">Pro Vault</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="learn" className="mt-4">
            <div className="grid gap-4 md:grid-cols-3">
                  ...
                  {CORE_MODULES.map((m) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full rounded-2xl border border-border px-3 py-3 text-left transition hover:shadow-sm ${active ? "bg-muted" : "bg-background"}`}
                      >
              ...
              <Card className="rounded-2xl md:col-span-2">
                <CardHeader>
                  <SectionTitle icon={module.icon} title={module.title} subtitle={module.why} right={<Badge variant="outline">{module.skills.join(" • ")}</Badge>} />
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <div className="rounded-2xl border border-border p-4">
                    <div className="mb-2 flex items-center justify-between gap-2">
                      ...
                      <Pill>Write facts, links, and decisions.</Pill>
                    </div>
                    <Textarea
          ...
          <TabsContent value="ship" className="mt-4">
            <div className="grid gap-4 md:grid-cols-2">
              <Card className="rounded-2xl">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="grid gap-2">
                    ...
                    <Textarea value={blueprint.goal} onChange={(e) => setBlueprint((b) => ({ ...b, goal: e.target.value }))} />
                  </div>

                  <div className="grid gap-2">
                    <div className="text-sm font-semibold">Environment</div>
                    <Textarea value={blueprint.environment} onChange={(e) => setBlueprint((b) => ({ ...b, environment: e.target.value }))} />
                  </div>
                  ...
                  <div className="rounded-2xl bg-muted p-3">
                    ...
                    <pre className="mt-2 max-h-72 overflow-auto whitespace-pre-wrap rounded-xl bg-background p-3 text-xs leading-relaxed">{blueprintMarkdown}</pre>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="rounded-2xl">
                <CardHeader>
                  <SectionTitle icon={DollarSign} title="Tool stack (default recommendation)" subtitle="Choose boring, reliable primitives. Fancy comes later." />
                </CardHeader>
                <CardContent className="space-y-4">
                  {TOOL_STACK.map((layer) => (
                    <div key={layer.layer} className="rounded-2xl border border-border p-4">
            ...
            <Card className="rounded-2xl">
              <CardHeader>
                <SectionTitle
                  ...
                  subtitle="Designed for 2026: agentic, multimodal, compliance-aware, measurable ROI."
                />
              </CardHeader>
          ...
          <TabsContent value="social" className="mt-4">
            <div className="grid gap-4 md:grid-cols-2">
              <Card className="rounded-2xl">
                <CardHeader>
                  <SectionTitle
                    ...
                    subtitle="Build from scratch or acquire legitimately (partnership, purchase, or hired admin). No takeovers."
                  />
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid gap-2">
              ...
              <Card className="rounded-2xl">
                <CardHeader>
                  <SectionTitle icon={Zap} title="Tonight’s content generator" subtitle="Paste a topic. It outputs 10 hooks, 3 scripts, and 1 offer angle." />
                </CardHeader>
                <CardContent className="space-y-3">
                  <ContentGen />
                </CardContent>
                  ...
                  {(state.intelPacks || []).length === 0 ? (
                    ...
                    (state.intelPacks || []).map((p) => (
                      <div key={p.id} className="rounded-2xl border border-border p-4">
                        ...
                        {p.opportunity ? <div className="mt-2 text-sm">Opportunity: {p.opportunity}</div> : null}
                        ...
                        {Array.isArray(p.links) && p.links.length ? (
                          <div className="mt-3 text-xs text-muted-foreground">Links: {p.links.slice(0, 3).join(" • ")}</div>
                        ) : null}
                      </div>
                    ))
                  )}
            ...
            <Card className="rounded-2xl">
              <CardHeader>
                <SectionTitle
                  icon={KeyRound}
                  title="Pro Vault"
                  subtitle="Sellable ‘drops’ you can put behind an access key. MVP is local-only unlock."
                  right={
                    <Button variant="secondary" className="gap-2" onClick={() => setShowGate(true)}>
              ...
              <CardContent className="space-y-4">
                {!proUnlocked ? (
                  <div className="rounded-2xl bg-muted p-4">
                    ...
                    <div className="mt-1 text-sm text-muted-foreground">Unlock to access the prompt packs and premium templates.</div>
                  </div>
                ) : (
                  <div className="grid gap-3">
                    {Object.entries(PRO_PROMPTS).map(([k, p]) => (
                      <div key={k} className="rounded-2xl border border-border p-4">
                        <div className="flex items-start justify-between gap-2">
                          <div className="font-semibold">{p.title}</div>
                          <Button size="sm" variant="secondary" className="gap-2" onClick={() => copy(p.body)}>
                            <Copy className="h-4 w-4" /> Copy
                          </Button>
                        </div>
                        <pre className="mt-3 whitespace-pre-wrap rounded-xl bg-muted p-3 text-xs leading-relaxed">{p.body}</pre>
                      </div>
                    ))}
                  </div>
                )}
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex gap-2">
        ...
        {out ? (
          <Button variant="secondary" className="gap-2" onClick={() => navigator.clipboard.writeText(JSON.stringify(out, null, 2))}>
